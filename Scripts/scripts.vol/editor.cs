...
function ME::RegisterObject(%group, %name, %func, %a1, %a2, %a3, %a4, %a5, %a6, %a7, %a8, %a9, %a10)
{
   if($ME::NumGroups == "")
      $ME::NumGroups = 0;
...
function MEShowCreator()
{
   ...
   unfocus(TedObject);
   focus(MissionEditor);
   playerFocusCamera($ME::camera);
   postAction(EditorGui, Attach, $ME::camera);
   cursorOn(simCanvas);
...
function MEMode()
{
   ...
   if(!isObject(EditorGui))
      GuiLoadContentCtrl("simCanvas", "editor.gui");
...
//   eval(Terrain:: @ $terrainTypes[%tt, type] @ "::create(" @ %ts @ "," @ $ME::terrainSeed @ ");");
   eval(Terrain:: @ $terrainTypes[%tt, type] @ "::create(" @ $ME::terrainSeed @ ");");
   LS::parseCommands();
   ...
   if($ME::genRandSeed == true)
      $ME::terrainSeed = randomInt(0,65535);
